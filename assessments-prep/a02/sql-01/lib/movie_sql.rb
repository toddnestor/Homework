...
def biggest_years_for_little_danny
  MovieDatabase.execute(<<-SQL)
    SELECT
      movies.yr, COUNT(DISTINCT movies.id) as count
    FROM
      movies
      JOIN castings ON castings.movie_id = movies.id
      JOIN actors ON castings.actor_id = actors.id
...
def twenty_roles
  MovieDatabase.execute(<<-SQL)
    ...
    GROUP BY
      actors.id
    HAVING
      COUNT(DISTINCT castings.movie_id) = 20
    ORDER BY
      actors.name
  SQL
end
...
def chris_is_missed
  MovieDatabase.execute(<<-SQL)
    ...
    FROM
      ...
      JOIN castings ON movies.id = castings.movie_id
      JOIN actors ON actors.id = castings.actor_id
      JOIN castings AS farley_castings ON movies.id = farley_castings.movie_id
      JOIN actors AS farley ON farley.id = farley_castings.actor_id
    WHERE
      farley.name = 'Chris Farley' AND castings.ord = 1
    GROUP BY
      movies.id
    ORDER BY
      movies.title
