...
def biggest_years_for_little_danny
  MovieDatabase.execute(<<-SQL)
    SELECT
      movies.yr, COUNT(castings.movie_id) as count
    FROM
      movies
      JOIN castings ON castings.movie_id = movies.id
      JOIN actors ON castings.actor_id = actors.id
...
def twenty_roles
  MovieDatabase.execute(<<-SQL)
    ...
    FROM
      actors
      JOIN castings ON castings.actor_id = actors.id AND castings.ord = 1
    GROUP BY
      actors.id
    HAVING
...
def chris_is_missed
  MovieDatabase.execute(<<-SQL)
    ...
    FROM
      movies
      JOIN castings ON castings.movie_id = movies.id AND castings.ord = 1
      JOIN actors ON castings.actor_id = actors.id
      JOIN castings farley_roles ON farley_roles.movie_id = movies.id
      JOIN actors farley ON farley.id = farley_roles.actor_id AND farley.name = 'Chris Farley'
    GROUP BY
      movies.id
  SQL
end
